...
impl Joypad {
    ...
    pub fn update(&mut self, mmu: &mut MMU) {
        let old_value = self.joypad_state;
        let new_value = self.get_joypad_state();
        ...
        self.joypad_state = new_value;
    }

    fn get_joypad_state(&self) -> u8 {
        let mut result = 0xCF; // 高兩位固定為 1

        // 方向鍵被選中
        if (self.select_direction & 0x10) == 0 {
            if self.right {
                result &= !(1 << 0);
            }
            if self.left {
